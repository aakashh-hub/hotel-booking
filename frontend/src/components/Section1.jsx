...
const Section1 = ({ next }) => {
  ...
  const [basicInfo, setBasicInfo] = useState(formData.basicInfo || {
    hotelName: '',
    description: '',
    photos: [],
  });
  ...
  const handleNext = () => {
    if (basicInfo.hotelName && basicInfo.description && basicInfo.photos.length > 0) {
